...
class Case extends Equatable {
  ...
  factory Case.merge(Case value, Case other) {
    assert(value.condition == other.condition, 'Two merge cases should have the same condition');

    final result = Case(
      condition: value.condition,
      translations: [
        ...value.translations,
        ...other.translations.where((o) => !value.translations.any((v) => v.languageCode == o.languageCode))
      ],
    );
  ...
  List<StringTemplate> get templatedValues {
    ...
    return translations.isEmpty ? const <StringTemplate>[] : translations.first.templatedValues;
  }
