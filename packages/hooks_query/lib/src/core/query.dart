...
class Query<TData, TError>
  ...
  Future<TData> fetch(
    ...
    try {
      ...
      // Only (re)schedule garbage collection while the query is unobserved.
      // An observed query must not hold a pending gc timer — besides being
      // wrong (it can never be collected while observed), the lingering timer
      // distorts fake_async time in tests.
      if (!hasObservers) {
