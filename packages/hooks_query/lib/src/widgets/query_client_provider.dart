...
class QueryClientProvider extends StatefulWidget {
  ...
  static QueryClient of(BuildContext context) {
    final accessor = context.getOrNull<QueryClient Function()>();
    if (accessor == null) {
      throw FlutterError(
  ...
  static QueryClient? maybeOf(BuildContext context) {
    return context.getOrNull<QueryClient Function()>()?.call();
  }
...
class _QueryClientProviderState extends State<QueryClientProvider> {
  ...
  // Stable accessor closure provided to descendants — avoids rebuilds on every
  // access and enables lazy creation for the lazy == true case.
  late final QueryClient Function() _accessor;
  ...
  void initState() {
    ...
    if (widget._value != null) {
      ...
      // Defer create() until first access via of() / maybeOf() / useQueryClient().
      _accessor = () {
        if (_client == null) {
  ...
  Widget build(BuildContext context) {
    return ValueProvider<QueryClient Function()>(_accessor, child: widget.child);
  }
}
